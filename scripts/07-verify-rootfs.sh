...
require_content etc/fstab '/var/run'
require_content etc/fstab '/var/lock'
...
require_content opt/x-chip-boot.sh 'Console and SSH recovery ready'
...
reject_content opt/x-chip-boot.sh 'start_desktop'
reject_content opt/x-chip-boot.sh 'x-chip-desktop-start'
...
require_content usr/local/bin/x-chip-brightness 'LCD_BRIGHTNESS'
...
reject_content tce/media.lst 'Xorg.tcz'
reject_entry tce/xorg.lst
...
for shell_script in \
    ...
    require_shell_syntax "$shell_script"
done
